    ...
    return (<StickyBox offsetTop={20} offsetBottom={20} {...props}>
        ...
        <ModalPreviewGiftCard amount={values.amount} message={values.message} />
            ...
            <div className="form-group">
                {!(props.isValid && values.isAgree) ? <PayPalDisabled/> : <PaymentButton {...props} typeApi="giftCard" values={values} handlerSubmit={handlerSubmit} total={values.amount} />}
            </div>
        </div>
    </StickyBox>);
}
                ...
                <div className="col-md-12 col-lg-7 col-sm-12 col-xs-12">
                    ...
                    <div className="row ">
                            ...
                            {amounts.map( (amount, index) => {
                                return <div key={index} className={`col  ${ (amount === 400 || 600 || 2000) ? "" : "" } `}>
                                        <button style={{width: '100%'}} type="button" className="btn btn-outline-secondary button-amount-check" onClick={()=>{
                                    setFieldValue('amount', amount);
                                }}>{amount}₪</button></div>
                            })}
                        <div className="col">
                            <Field name="amount" type="number"  id="amount" min={200} component={Input} label="Amount*" />
                    ...
                    <div className="row">
                        ...
                        <div className="col-md-6 col-xs-12  mb-1">
                            <div className="input-group">
                                <Field id="billingAddress_phone" name="billingAddress_phone" type="text" placefullName="Phone" label="Phone*"   component={Input} />
...
export default withFormik({
      ...
      handleSubmit: async (values, {props, setSubmitting, resetForm, setStatus, setErrors}) =>  {
            ...
            axios.put("/api/giftCard", values)
              .then(function (response) {
                if(response.status === 200) {
                    axios.post("/api/sendmail/giftCard", values)
                        .then(function (response){
                            if(response.status === 200) {
                                setSubmitting(false);
                                resetForm();
